...
# @sacloud-text maxlen=18 route0_prefix1 "ROUTE1 PREFIX1(*.*.*.*/*)"
# @sacloud-text maxlen=15 route0_gateway1 "ROUTE1 GATEWAY1(*.*.*.*)"
...
ETH1_IP=@@@eth1_ip@@@ 	      	      # *.*.*.*/**
ROUTE0_PREFIX1=@@@route0_prefix1@@@   # *.*.*.*/**
ROUTE0_GATEWAY1=@@@route0_gateway1@@@ # *.*.*.*
ROUTE1_PREFIX1=@@@route1_prefix1@@@   # *.*.*.*/**
ROUTE1_GATEWAY1=@@@route1_gateway1@@@ # *.*.*.*
UPDATE=@@@update@@@ 	      	      # yes
...
function setup_route0_1() {
  PREFIX=$ROUTE0_PREFIX1
  GATEWAY=$ROUTE0_GATEWAY1
  if [ "$PREFIX" == "" ]; then return 0; fi
  nmcli con mod "System eth0" \
  +ipv4.routes "${PREFIX} ${GATEWAY}"
  nmcli con down "System eth0"; nmcli con up "System eth0"
}
setup_route0_1

function setup_route1_1() {
  PREFIX=$ROUTE1_PREFIX1
  GATEWAY=$ROUTE1_GATEWAY1
  if [ "$PREFIX" == "" ]; then return 0; fi
  nmcli con mod "System eth1" \
  +ipv4.routes "${PREFIX} ${GATEWAY}"
  nmcli con down "System eth1"; nmcli con up "System eth1"
